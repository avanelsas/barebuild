...
- **Pure**: Every decision lives in `step` and is visible in the returned effects.
  ...
  `step` is testable and replayable from an event log (see [BareReplay](https://github.com/avanelsas/barereplay)).
...
Lint: `clj-kondo --lint src test demo/src demo/test demo/dev-server`.

BareBuild builds its own custom elements on three utility namespaces from
`com.github.avanelsas/baredom`: `utils.component`, `utils.dom` and `utils.model`. The
version is declared in both `shadow-cljs.edn` and `deps.edn` and the two must agree, or a
local build passes while every consumer's fails.

That is an implementation dependency, not a limit on what BareBuild can drive. A consumer
reaches its child with a plain CSS selector and projects onto attributes and properties, so
any component taking those will do. An app whose consumers drive BareDOM components
installs `@vanelsas/baredom` itself, as its own dependency.

### Release

Bump the version in `package.json`, `deps.edn` and `build.clj`, add a `CHANGELOG.md` entry,
then tag `vX.Y.Z`. The release workflow refuses to run unless all three files match the tag,
then lints, tests, builds, publishes the jar to Clojars, and creates the GitHub Release.

Running the showcase demo (a live page driving components from a tasks server) has its own
guide, see [`demo/README.md`](./demo/README.md).
